...
public class MessageHandler {
    ...
    private void addUserToChannel(JSONObject jsonRequest, WebSocketSession session){
        ...
        try {
            if(WebSocketSessionManager.getInstance().getSession(userId) != null) {
                WebSocketSessionManager.getInstance().getSession(userId).sendMessage(new TextMessage(userResponse.toString()));
            }
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
